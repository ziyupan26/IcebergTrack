...
def make_plot(arquivo_path, image_name, segOutput, img_result, land_pixels, area_pixels, idf_validos, data_img,
    ...
    # Inserindo metadados sobre a imagem 插入有关图像的元数据
    if fullplot:
        if showlabel:
            N_label = 1
            for i in idf:
        ...
        # Plotando Usando Basemap 使用底图绘图
        plt.clf()
        plt.close("all")
        ...
        m = Basemap(llcrnrlon=lllon, llcrnrlat=lllat, urcrnrlon=urlon, urcrnrlat=urlat, epsg=3262) # 32661
